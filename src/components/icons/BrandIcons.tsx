    ...
    >
      <defs>
        <linearGradient id={gradId} x1="0%" y1="0%" x2="0%" y2="100%">
          <stop offset="0%" stopColor="#A3AEFA" />
          <stop offset="55%" stopColor="#7080F2" />
          <stop offset="100%" stopColor="#4755E3" />
        </linearGradient>
      </defs>
      {/* 蓝紫渐变 squircle 底板 */}
      <rect x="0.5" y="0.5" width="23" height="23" rx="7" fill={`url(#${gradId})`} />
      {/* 白色终端提示符 ">_" */}
      <path
        d="M7.1 8.4 L10.5 12 L7.1 15.6"
        stroke="#FFFFFF"
        strokeWidth="2.1"
        strokeLinecap="round"
 ...
 * The authentic four-pointed curved celestial star with Google's multi-stop rainbow gradient
 */
export const GeminiIcon: React.FC<IconProps> = ({ className = 'w-5 h-5', size = 20, ...props }) => {
  const gradientId = 'official-gemini-gradient';
  return (
    <svg
      viewBox="0 0 24 24"
      className={className}
...
export const OpenClawIcon: React.FC<IconProps> = ({ className = 'w-5 h-5', size = 20, ...props }) => {
  const bodyGrad = 'openclaw-body-grad';
  const clawGrad = 'openclaw-claw-grad';
  return (
    <svg
      viewBox="0 0 100 100"
      fill="none"
    ...
    >
      <defs>
        <radialGradient id={bodyGrad} cx="45%" cy="38%" r="58%">
      ...
      />
      {/* 大红圆形躯干 */}
      <circle cx="50" cy="55" r="28" fill={`url(#${bodyGrad})`} />
      {/* 青色圆溜溜发光双眼 */}
 ...
 * 官方矢量字形（来源：antigravity.google/press，simple-icons 标准化），Google 蓝渐变
 */
export const AntigravityIcon: React.FC<IconProps> = ({ className = 'w-5 h-5', size = 20, ...props }) => {
  const gradId = 'antigravity-official-grad';
  return (
    <svg
      viewBox="0 0 24 24"
      fill="none"
...
export const ToolBrandIcon: React.FC<{ toolId: string; size?: number; className?: string }> = ({
  ...
  switch (toolId) {
    ...
    case 'codex':
      return (
        <span
          className={`inline-flex items-center justify-center rounded-lg overflow-hidden shadow-2xs ${className}`}
          title="OpenAI Codex"
        >
          <CodexIcon size={size} />
        </span>
...
/**
 * 技能坞 (SkillDock) 官方专属高质感矢量品牌徽标
 * 结合立体等轴测菱形平台与多层 Dock 停靠轨结构
 */
export const SkillDockLogo: React.FC<{ size?: number; className?: string }> = ({
  ...
  return (
    <div
      className={`relative flex items-center justify-center rounded-xl bg-gradient-to-br from-indigo-500 via-indigo-600 to-violet-700 p-1.5 shadow-md shadow-indigo-500/25 border border-indigo-400/40 group-hover:scale-105 group-hover:shadow-indigo-500/40 transition-all duration-200 shrink-0 ${className}`}
    ...
    >
      ...
      >
        <defs>
          <linearGradient id="dock-facet-top" x1="0%" y1="0%" x2="100%" y2="100%">
            <stop offset="0%" stopColor="#FFFFFF" stopOpacity="0.98" />
            <stop offset="100%" stopColor="#C7D2FE" stopOpacity="0.85" />
          </linearGradient>
          <linearGradient id="dock-facet-shelf" x1="0%" y1="0%" x2="100%" y2="0%">
        ...
        {/* 顶部等轴测停靠平台 */}
        <path
          d="M12 2.5L20.5 7.2L12 11.9L3.5 7.2L12 2.5Z"
          fill="url(#dock-facet-top)"
        />
        {/* 核心能量晶片槽位 */}
        <path
          d="M12 5.5L15 7.2L12 8.9L9 7.2L12 5.5Z"
          fill="#4F46E5"
        />
        {/* 第二层扩展轨道 */}
        <path
          d="M3.5 11.8L12 16.5L20.5 11.8"
          stroke="url(#dock-facet-shelf)"
          strokeWidth="2.2"
          strokeLinecap="round"
          strokeLinejoin="round"
        />
